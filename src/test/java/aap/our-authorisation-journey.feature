Feature: I can haz permissions
  ...
  Background:
    * url baseUrl
    * def alice = callonce read('classpath:user-create.feature') { username: 'test-alice' }
  ...
  Scenario: Anyone can create a domain
    ...
    When def domain = call read('classpath:domain-create.feature') { name: '#(domainName)', manager: '#(alice.token)', member: '#(rabbit.reference)' }
